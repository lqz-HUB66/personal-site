"use client";

import { useState } from "react";
import { useRouter } from "next/navigation";
import { motion } from "framer-motion";
import ImageUpload from "./image-upload";

interface HonorData {
  title: string;
  level: string;
  organizer: string;
  date: string;
  description: string;
  image: string;
  tags: string;
  year: string;
  isFeatured: boolean;
}

interface Props {
  initialData?: HonorData;
  honorId?: string;
  mode: "create" | "edit";
}

const defaultData: HonorData = {
  title: "",
  level: "",
  organizer: "",
  date: "",
  description: "",
  image: "",
  tags: "[]",
  year: new Date().getFullYear().toString(),
  isFeatured: false,
};

const inputClass =
  "w-full px-3 py-2 text-sm border border-[#222] rounded-md bg-[#0a0a0a] text-white focus:outline-none focus:border-[#444] transition-colors font-mono";

export default function HonorForm({ initialData, honorId, mode }: Props) {
  const router = useRouter();
  const [data, setData] = useState<HonorData>(initialData || defaultData);
  const [tagsInput, setTagsInput] = useState(
    initialData?.tags
      ? (JSON.parse(initialData.tags) as string[]).join(", ")
      : ""
  );
  const [saving, setSaving] = useState(false);
  const [error, setError] = useState("");

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    setSaving(true);

    const tags = tagsInput
      .split(",")
      .map((s) => s.trim())
      .filter(Boolean);

    const body = {
      ...data,
      tags: JSON.stringify(tags),
    };

    try {
      const url =
        mode === "create" ? "/api/honors" : `/api/honors/${honorId}`;
      const method = mode === "create" ? "POST" : "PUT";

      const res = await fetch(url, {
        method,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(body),
      });

      if (!res.ok) {
        const err = await res.json();
        setError(err.error || "保存失败");
        return;
      }

      router.push("/admin/honors");
    } catch {
      setError("保存失败");
    } finally {
      setSaving(false);
    }
  };

  return (
    <form onSubmit={handleSubmit} className="space-y-5 max-w-2xl">
      <div>
        <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
          荣誉名称 <span className="text-[#dc2626]">*</span>
        </label>
        <input
          type="text"
          value={data.title}
          onChange={(e) => setData({ ...data, title: e.target.value })}
          required
          placeholder="例：全国大学生信息安全竞赛一等奖"
          className={inputClass}
        />
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
            获奖等级
          </label>
          <input
            type="text"
            value={data.level}
            onChange={(e) => setData({ ...data, level: e.target.value })}
            placeholder="例：国家级一等奖"
            className={inputClass}
          />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">年份</label>
          <input
            type="text"
            value={data.year}
            onChange={(e) => setData({ ...data, year: e.target.value })}
            placeholder="2024"
            className={inputClass}
          />
        </div>
      </div>

      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
            主办方
          </label>
          <input
            type="text"
            value={data.organizer}
            onChange={(e) =>
              setData({ ...data, organizer: e.target.value })
            }
            placeholder="例：教育部"
            className={inputClass}
          />
        </div>
        <div>
          <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
            获奖时间
          </label>
          <input
            type="text"
            value={data.date}
            onChange={(e) => setData({ ...data, date: e.target.value })}
            placeholder="2024-05"
            className={inputClass}
          />
        </div>
      </div>

      <div>
        <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
          荣誉描述
        </label>
        <textarea
          value={data.description}
          onChange={(e) =>
            setData({ ...data, description: e.target.value })
          }
          rows={3}
          placeholder="简要描述这项荣誉"
          className={`${inputClass} resize-none`}
        />
      </div>

      <div>
        <label className="block text-sm font-medium mb-1.5 text-[#999] font-mono">
          标签（逗号分隔）
        </label>
        <input
          type="text"
          value={tagsInput}
          onChange={(e) => setTagsInput(e.target.value)}
          placeholder="CTF, 信息安全, 算法"
          className={inputClass}
        />
      </div>

      <ImageUpload
        value={data.image}
        onChange={(url) => setData({ ...data, image: url })}
        label="证书图片"
      />

      <label className="flex items-center gap-2 cursor-pointer">
        <input
          type="checkbox"
          checked={data.isFeatured}
          onChange={(e) =>
            setData({ ...data, isFeatured: e.target.checked })
          }
          className="rounded border-[#333] bg-[#0a0a0a]"
        />
        <span className="text-sm font-mono text-[#999]">在首页展示</span>
      </label>

      {error && (
        <p className="text-sm text-[#dc2626]">{error}</p>
      )}

      <div className="flex items-center gap-3 pt-2">
        <motion.button
          type="submit"
          disabled={saving}
          whileTap={{ scale: 0.97 }}
          className="px-5 py-2 text-sm font-medium text-black bg-white rounded-md hover:bg-[#e5e5e5] disabled:opacity-50 transition-colors font-mono"
        >
          {saving
            ? "保存中..."
            : mode === "create"
            ? "创建荣誉"
            : "保存修改"}
        </motion.button>
        <motion.button
          type="button"
          onClick={() => router.back()}
          whileTap={{ scale: 0.97 }}
          className="px-5 py-2 text-sm text-[#666] hover:text-white transition-colors font-mono"
        >
          取消
        </motion.button>
      </div>
    </form>
  );
}
